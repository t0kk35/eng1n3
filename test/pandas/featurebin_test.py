...
class TestFeatureBin(unittest.TestCase):
    def test_read_base_non_inference(self):
        ...
        with en.EnginePandas(num_threads=1) as e:
            ...
            self.assertEqual(df.iloc[:, 0].dtype.name, 'int16', f'Expecting a "int16" data type')
            ...
            self.assertEqual(len(fb.bins), len(fb), f'Length should be one bigger than bins (because of 0)')
            self.assertEqual(nr_bins, fb.number_of_bins, f'Number of bins not  {nr_bins}, got {fb.number_of_bins}')
            ...
            self.assertIsInstance(fb.index_to_label, dict, f'Expecting the index to label to be a dict')
            self.assertEqual(len(fb.index_to_label), nr_bins, f'Unexpected size of ItoL, got {len(fb.index_to_label)}')
            self.assertListEqual(sorted(list(fb.index_to_label.keys())), list(range(0, nr_bins)))
            ...
            self.assertEqual(sorted(list(df[bin_name].unique())), list(range(1, fb.number_of_bins)))
    ...
    def test_read_remove_element(self):
        ...
        with en.EnginePandas(num_threads=1) as e:
            ...
            self.assertEqual(df_1.iloc[:, 0].dtype.name, 'int16', f'Expecting a "int16" data type')
            self.assertEqual(fb.inference_ready, True, f'Bin feature should be ready for inference')
    ...
    def test_read_add_element_small(self):
        ...
        with en.EnginePandas(num_threads=1) as e:
            ...
            self.assertEqual(fb.number_of_bins, len(bin_v)+1, f'Number of bins changed {fb.number_of_bins}')
            self.assertEqual(df[bin_name][0], 1, f'Missing row should have been in 0 bin {df[bin_name][0]}')
            self.assertEqual(df[bin_name][1], 1, f'2 nd row should have been in 0 bin {df[bin_name][0]}')
    ...
    def test_read_add_element_large(self):
        ...
        with en.EnginePandas(num_threads=1) as e:
            ...
            df = e.df_from_csv(td, file1, inference=True)
            self.assertEqual(fb.number_of_bins, len(bin_v)+1, f'Number of bins changed {fb.number_of_bins}')
            self.assertEqual(df[bin_name][0], 1, f'First row should have been in 0 bin {df[bin_name][0]}')
            self.assertEqual(df[bin_name].iloc[-1], nr_bins-1, f'Last should have max bin {df[bin_name].iloc[-1]}')
            self.assertEqual(len(df.columns), 1, f'Should have gotten one column. Got {len(df.columns)}')
            self.assertEqual(fb.learning_category, ft.LEARNING_CATEGORY_CATEGORICAL, f'Expecting Categorical LC')
            self.assertEqual(df.columns[0], bin_name, f'Column name incorrect. Got {df.columns[0]}')
