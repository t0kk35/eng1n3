...
class TestIndex(unittest.TestCase):
    ...
    def test_read_base_non_inference(self):
        ...
        with en.EnginePandas(num_threads=1) as e:
            ...
            self.assertListEqual(sorted(list(fi.dictionary.keys())), sorted(list(mcc_v) + ['*_UNK_*']))
            self.assertIsInstance(fi.index_to_label, dict, f'Expecting the index to label to be a dict')
            self.assertEqual(len(fi.index_to_label), len(list(mcc_v))+1, f'Bad ItoL size, got {len(fi.index_to_label)}')
            self.assertListEqual(sorted(list(fi.index_to_label.keys())), list(range(0, len(list(mcc_v))+1)))
            self.assertListEqual(sorted(list(fi.index_to_label.values())), sorted(list(mcc_v) + ['*_UNK_*']))
    ...
    def test_inference_read_remove_element(self):
        ...
        with en.EnginePandas(num_threads=1) as e:
            ...
            self.assertEqual(len(fi), len(mcc_v) + 2, f'Length of dictionary changed {len(fi)}')
            self.assertEqual(fi.learning_category, ft.LEARNING_CATEGORY_CATEGORICAL, f'Expecting Categorical LC')
            self.assertEqual(len(fi.embedded_features), 1, f'Expecting 1 embedded ft. Got {len(fi.embedded_features)}')
            self.assertEqual(fi.embedded_features[0], fc, 'Embedded feature should be the original source feature')
            self.assertEqual(len(fi.dictionary), len(mcc_v) + 2, f'Dictionary length not correct {len(fi.dictionary)}')
            self.assertIsInstance(fi.index_to_label, dict, f'Expecting the index to label to be a dict')
            self.assertEqual(len(fi.index_to_label), len(list(mcc_v))+2, f'Bad ItoL size, got {len(fi.index_to_label)}')
            self.assertEqual(td.rank, 2, f'This should have been a rank 2 tensor. Got {td.rank}')
    ...
    def test_inference_read_add_element(self):
        ...
        with en.EnginePandas(num_threads=1) as e:
            ...
            self.assertEqual(len(fi.embedded_features), 1, f'Expecting 1 embedded ft. Got {len(fi.embedded_features)}')
            self.assertEqual(fi.embedded_features[0], fc, 'Embedded feature should be the original source feature')
            self.assertEqual(len(fi.dictionary), len(mcc_v) + 1, f'Length of dictionary changed {len(fi)}')
...
class TestNP(unittest.TestCase):
    def test_from_np_good(self):
        ...
        td1 = ft.TensorDefinition('TestNP', [fi])
        td2 = ft.TensorDefinition('TestNP', [fi])
        with en.EnginePandas(num_threads=1) as e:
