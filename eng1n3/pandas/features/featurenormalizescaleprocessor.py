...
from f3atur3s import Feature, FeatureNormalizeScale, FeatureTypeInteger
...
class FeatureNormalizeScaleProcessor(FeatureNormalizeProcessor[FeatureNormalizeScale]):
    ...
    def process(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for f in self.features:
            ...
            if log_fn is None:
                kwargs[fn] = (df[bfn] - f.minimum) / (f.maximum - f.minimum)
            else:
                kwargs[fn] = (log_fn(df[bfn]+f.delta) - f.minimum) / (f.maximum - f.minimum)
